...
def percentageGoldGained(gold_gained):
    total_avaiable = (
        ((3 * 34 + 43) * 10 * 2) - 1
    ) + 59  # Gold from creepwaves -1 for the last round that the wave spawns on +59 siege creep
    total_avaiable = total_avaiable + (100 * 5) + (106 * 5)  # Passive gold calculation
    total_avaiable = (
        total_avaiable + 80 + (36 * 2) + 45
    )  # Runes that are avaible for one team before 10 min
    ...
    player_resp = {
        ...
        "predicted_win": round(
            predictModel(
                rank,
                player["ml_lane_role"],
                [positionFiller(player, player["ml_lane_role"])],
            )
            * 100,
            3,
        ),
